...
test.describe('Onboarding Flow', () => {
  ...
  test('should complete onboarding flow successfully', async ({ page }) => {
    ...
    const bottomNav = page.locator('[data-testid="bottom-navigation"], .bottom-navigation, button:has-text("Today"), button:has-text("Plan"), button:has-text("Chat")');
  ...
  test('should handle onboarding completion errors gracefully', async ({ page }) => {
    ...
    // Click button and expect fallback completion
    await onboardingButton.click();
    await page.waitForTimeout(3000);
    
    // Even with API errors, should still complete
    const completionState = await page.evaluate(() => {
      return localStorage.getItem('onboarding-complete');
    });
    expect(completionState).toBe('true');
    console.log('✅ Fallback completion worked');
  });
