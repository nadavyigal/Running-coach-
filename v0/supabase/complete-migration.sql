-- Complete Supabase Migration for Running Coach App (Story 9.4)
...
-- Step 4: Create unique constraint for single active plan per user
DO $$ BEGIN
...
EXCEPTION
    WHEN duplicate_table THEN 
        RAISE NOTICE 'Index idx_plans_single_active already exists, skipping';
END $$;

-- Step 5: Create workouts table
...
-- Step 7: Create conversation_messages table
...
-- Step 8: Create idempotency_keys table
...
-- Step 14: Create RLS policies for plans
DO $$ BEGIN
...
-- Step 15: Create RLS policies for workouts (similar pattern)
DO $$ BEGIN
...
-- Step 16: Create RLS policies for conversations
DO $$ BEGIN
...
-- Step 17: Create RLS policies for conversation_messages
DO $$ BEGIN
...
-- Step 18: Create RLS policies for idempotency_keys
DO $$ BEGIN
...
-- Step 19: Create the finalize_onboarding RPC function
...
-- Step 20: Create cleanup function for expired idempotency keys
...
DO $$ BEGIN
    ...
    RAISE NOTICE 'Tables created: profiles, plans, workouts, conversations, conversation_messages, idempotency_keys';
    RAISE NOTICE 'RLS policies enabled for all tables';
    RAISE NOTICE 'finalize_onboarding function created';
