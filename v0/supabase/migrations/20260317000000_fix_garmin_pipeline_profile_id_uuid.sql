-- Fix garmin auto-sync pipeline: corrects profile_id bigint → uuid type mismatch
-- The 20260310110000 migration failed because profile_id was declared bigint but
-- profiles.id is UUID. This migration re-applies everything from that migration
-- with the correct type, using IF NOT EXISTS / DO blocks to be idempotent.

begin;

-- ─── garmin_connections extra columns ──────────────────────────────────────
...
-- back-fill provider_user_id from garmin_user_id (if that column exists)
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public'
      and table_name  = 'garmin_connections'
      and column_name = 'garmin_user_id'
  ) then
...
-- back-fill profile_id from auth_user_id → profiles join
...
-- back-fill last_successful_sync_at from last_sync_at (if that column exists)
do $$
begin
  if exists (
    select 1 from information_schema.columns
...
create table if not exists public.garmin_import_jobs (
  ...
  user_id          bigint,
  -- profile_id is UUID to match profiles(id) primary key type
...
-- ─── runs source columns (idempotent) ───────────────────────────────────────
