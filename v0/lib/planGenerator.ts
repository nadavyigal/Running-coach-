import { type Plan, type User, type Workout } from './db'
...
export interface GeneratePlanOptions {
  ...
  totalWeeks?: number
  planPreferences?: Record<string, any>
}
...
export async function generatePlan(options: GeneratePlanOptions): Promise<PlanData> {
  ...
  try {
    ...
    const resolvedTotalWeeks =
      typeof totalWeeks === 'number' ? totalWeeks : (generatedPlan.totalWeeks as number)
    const endDate = new Date(startDate)
    endDate.setDate(endDate.getDate() + resolvedTotalWeeks * 7)
    ...
    for (const workoutData of generatedPlan.workouts.filter(
      (w: any) => w.week >= 1 && w.week <= resolvedTotalWeeks
    )) {
 ...
 * Ensures the date is never scheduled before the plan start date.
 */
...
export async function generateFallbackPlan(
  ...
  options?: { totalWeeks?: number; planPreferences?: Record<string, any> }
): Promise<PlanData> {
  const totalWeeks =
    options?.totalWeeks ??
    (rookie_challenge
  ...
  const workouts: Omit<Workout, 'id' | 'createdAt' | 'updatedAt'>[] = []
  const preferredDaysRaw = options?.planPreferences?.trainingDays || options?.planPreferences?.preferredDays
  const longRunDay = options?.planPreferences?.longRunDay
  const preferredDays = Array.isArray(preferredDaysRaw)
    ? preferredDaysRaw.filter((d: any) => typeof d === 'string')
    : null
  ...
  const selectedDays =
    preferredDays && preferredDays.length >= (user.daysPerWeek || 3)
      ? preferredDays.slice(0, user.daysPerWeek || 3)
      : defaultPattern.slice(0, user.daysPerWeek || 3)

  const normalizedLongRunDay =
    typeof longRunDay === 'string' && selectedDays.includes(longRunDay)
      ? longRunDay
      : selectedDays[selectedDays.length - 1]

  const weeklyPattern = selectedDays
    .map((day: string) => ({
      day,
      type: day === normalizedLongRunDay ? 'long' : 'easy',
    }))
    .map((workout, idx) => {
      if (workout.type === 'long') return workout
      if (idx === 1 && selectedDays.length >= 3) {
        return { ...workout, type: totalWeeks % 2 === 0 ? 'tempo' : 'intervals' }
      }
      return workout
    })
  ...
  for (let week = 1; week <= totalWeeks; week++) {
    for (const workout of weeklyPattern) {
      ...
      const distance =
        workout.type === 'long'
          ? baseDistance * 1.5 + (week - 1) * 0.5
          : baseDistance + (week - 1) * 0.3
      ...
      workouts.push({
        ...
        completed: false,
        scheduledDate,
        notes: getWorkoutNotes(workout.type),
      })
    }
  }
...
function getWorkoutNotes(type: string): string {
  ...
  return notes[type] || 'Follow your training plan and listen to your body.'
}
