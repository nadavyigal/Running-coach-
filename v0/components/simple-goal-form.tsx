...
import {
  Target, TrendingUp, CheckCircle2, Info,
...
export function SimpleGoalForm({ isOpen, onClose, userId, onGoalCreated }: SimpleGoalFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const estimatedDuration = Math.ceil((deadline.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));

      // Prepare goal data with conditional properties
      const baselineVal = currentLevel ? parseFloat(currentLevel) : parsedTargetValue * 1.2; // Default to 20% worse than target
      ...
      // Only add relevantContext if motivation is provided
      if (motivation) {
        goalData.relevantContext = motivation;
      }
      ...
      // Show toast for goal creation success
      toast({
        title: `Goal created! SMART score: ${validation.smartScore}/100`,
        description: `${validation.autoGenerated.length} fields auto-generated.`
      });
      ...
      // Get the created goal to pass to callback using the correct function
