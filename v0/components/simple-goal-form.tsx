  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.details || 'Failed to create goal');
      }
      ...
      toast({
        title: '✓ Goal created!',
        description: `SMART score: ${result.validation.smartScore}/100. ${result.validation.autoGenerated.length} fields auto-generated.`
      });
      ...
      try {
        const planResponse = await fetch('/api/generate-plan', {
          ...
          body: JSON.stringify({
            ...
            goalContext: {
              type: goalPayload.goalType,
              target: targetValue,
              deadline: deadline,
              metric: goalPayload.specificTarget.metric
            }
          })
        });
